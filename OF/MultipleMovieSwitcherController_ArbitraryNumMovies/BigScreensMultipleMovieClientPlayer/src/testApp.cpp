...
void testApp::setup(){
    ...
    for(int i = 0; i < numMovies; i++) {
        std::cout << movieFiles.at(i);
        
        movies.at(i).loadMovie(movieFiles.at(i));
        movies.at(i).play();
        movies.at(i).update();
    }
    state = 0;
...
void testApp::update(){
    ...
    // Pause non-playing movies, otherwise sound keeps going!
    for(int i = 0; i < numMovies; i++) {
        movies.at(i).setPaused(state != i);
    }
    
	if (started) {
		movies.at(state).update();
	}
...
void testApp::loadSettings(string fileString){
    ...
    numMovies = xmlReader.getNumTags("movie");
    cout << "loaded " << numMovies << " movies.\n";
    movies.resize(numMovies);
    movieFiles.resize(numMovies);
    for(int i = 0; i < numMovies; i++) {
        string filename = xmlReader.getValue("movie:","test",i);
        movieFiles.at(i) = ((char *) malloc(sizeof(char)*filename.length()));
        strcpy(movieFiles.at(i), filename.c_str());
